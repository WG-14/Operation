...
Reports must therefore expose `claims_scope`, `state_coverage_matrix`, and `outcome`. Non-flat or lifecycle-specific states such as `open_exposure`, `reserved_exit_pending`, `dust_only`, `non_executable_position`, `recovery_blocked`, `runtime_position_state_not_research_comparable`, and `research_model_lacks_lot_native_authority` are expected to fail closed unless a future `lot_native_simulation_v1` model explicitly implements and tests them. `FAIL_CLOSED_UNMODELED_STATE` is safe behavior, but it is not profile-transition evidence and is not evidence of full lifecycle equivalence.
