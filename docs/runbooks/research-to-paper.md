...
Review `claims_scope`, `state_coverage_matrix`, and `outcome` before using the report in a profile transition. `PASS_POSITIVE_EQUIVALENCE` supports only explicitly modeled state classes; currently that means `flat_no_dust_no_position` unless `lot_native_simulation_v1` has been implemented and tested. `FAIL_CLOSED_UNMODELED_STATE` for dust, residue, reserved-exit, open-exposure, non-executable-position, or recovery-blocked states preserves safety but is not full lifecycle equivalence evidence. Do not treat flat-only or signal-only equivalence as proof that paper/runtime order lifecycle, dust handling, reserved exits, or restart recovery match research.
