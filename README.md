...
Runtime persistence records a materialized strategy-set manifest before the decision/allocation/execution chain. The manifest includes active instance ids, raw/materialized parameters, parameter source/audit, approved profile bindings, strategy/runtime/plugin contract hashes, execution and risk config hashes, market scope, exposure-cap semantics, deterministic run-start request hashes, and `single_pair_runtime_enforced=true`. Decision bundles, portfolio allocations, and execution plans link to the same manifest id/hash. In multi-strategy mode any representative strategy decision persisted for compatibility remains explicitly non-authoritative; the replay authority chain is manifest -> decision bundle -> allocation -> portfolio target -> execution plan.
