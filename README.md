...
Promotion-grade decision-equivalence evidence must be generated through the repo-owned `research-export-decisions --profile` and `runtime-replay-decisions` commands, then compared as validated export wrappers. Manual JSON decision arrays are diagnostic only. Promotion-grade decision exports must include explicit `position_authority` with a state class and hashes matching the decision's `position_state_hash`, `order_rules_hash`, and `fee_authority_hash`. The currently supported positive equivalence baseline is explicitly modeled flat/no-dust/no-position state. Runtime-only dust, residue, reserved-exit, open-exposure, non-executable-position, or recovery-blocked states fail closed unless explicitly modeled. A fail-closed unmodeled state is safe, but it is not transition evidence and is not evidence of full research/paper/runtime lifecycle equivalence.

Decision-equivalence reports include `claims_scope`, `state_coverage_matrix`, and an `outcome` such as `PASS_POSITIVE_EQUIVALENCE`, `FAIL_CLOSED_UNMODELED_STATE`, `FAIL_ACTUAL_DRIFT`, `FAIL_INCOMPLETE_CANONICAL_PAYLOAD`, or `FAIL_EXPORT_BINDING`. Profile transitions require scope-aware reports with `outcome=PASS_POSITIVE_EQUIVALENCE`, `ok=true`, `promotion_grade_comparison=true`, no unsupported state classes, and no fail-closed unmodeled states. Operators must distinguish flat-only or signal-only equivalence from full lifecycle equivalence; current reports must not be read as proving lot-native lifecycle equivalence unless `claims_scope.full_lifecycle_equivalence_supported=true`.
