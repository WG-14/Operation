...
from bithumb_bot.research.strategy_registry import ResearchStrategyPlugin, strategy_runtime_capability_issues
from bithumb_bot.runtime_strategy_decision import get_runtime_decision_adapter, is_runtime_strategy_decision_result
...
def assert_replay_compatible_contract(
    ...
    *,
    ...
    through_ts = _seed_replay_db(tmp_path / f"{plugin.name}.sqlite", dataset)
    conn = sqlite3.connect(f"file:{(tmp_path / f'{plugin.name}.sqlite').resolve()}?mode=ro", uri=True)
