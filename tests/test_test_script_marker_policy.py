...
def test_operation_ci_and_runner_reference_only_operation_tests() -> None:
    workflow = Path(".github/workflows/operation-ci.yml").read_text(encoding="utf-8")
    runner = Path("scripts/run_operation_tests.sh").read_text(encoding="utf-8")

    assert "name: operation-ci" in workflow
    for key in (
        "BITHUMB_API_KEY: \"\"",
        "BITHUMB_API_SECRET: \"\"",
    ...
    referenced_tests = re.findall(r"^  (tests/[^\s]+\.py)$", runner, flags=re.MULTILINE)
