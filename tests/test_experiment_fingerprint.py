...
def test_experiment_fingerprint_is_stable_for_same_runtime_contract() -> None:
    assert experiment_fingerprint(strategy_name="canary_non_sma") == experiment_fingerprint(
...
def test_experiment_fingerprint_payload_uses_contract_hash_identity() -> None:
    payload = build_experiment_fingerprint_payload(strategy_name="canary_non_sma")
