...
def compute_strategy_decision_snapshot(*args, **kwargs):
    import operation.runtime_decision_service as runtime_decision_service
...
def __getattr__(name: str):
    if name == "build_execution_decision_summary":
        import operation.execution_service as execution_service
...
def prepare_strategy_decision_persistence_context(*args, **kwargs):
    import operation.run_loop_execution_planner as run_loop_execution_planner
...
def resolve_typed_execution_submit_expectation(summary):
    import operation.runtime.execution_coordinator as execution_coordinator
