...
def validate_buy_price_none_submit_contract(*, submit_contract: BuyPriceNoneSubmitContract) -> None:
    ...
    raise BrokerRejectError(
        "configured broker order-rule capability rejected BUY price=None before submit: "
...
def validate_order_rules_support(
    *,
    rules,
    side: str,
    order_type: str,
    ...
    if supported_sides and normalized_side not in supported_sides:
        raise BrokerRejectError(
            "configured broker order-rule capability rejected order side before submit: "
    ...
    if supported_types and normalized_order_type not in supported_types:
        raise BrokerRejectError(
            "configured broker order-rule capability rejected order type before submit: "
            f"order_type={normalized_order_type} supported={sorted(set(supported_types))}"
        )


def validate_buy_price_none_order_rules_contract(
    *,
    rules,
