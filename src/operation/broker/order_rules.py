...
from .availability import UnavailableBrokerFactory
from .buy_price_none_policy import (
    ...
    validate_buy_price_none_order_rules_contract,
    validate_buy_price_none_submit_contract,
    validate_order_rules_support,
)


class SourceOrderRulesMarketMismatchError(RuntimeError):
    pass


class SourceOrderRulesSchemaError(RuntimeError):
    pass


def _unsupported_rule_source(*_: object, **__: object):
    raise RuntimeError("live broker order-rule discovery is unavailable")


ExchangeDerivedConstraints = object
OrderRulesResponse = object
OrderRulesSide = object
derive_order_rules_from_chance = _unsupported_rule_source
parse_order_rules_response = _unsupported_rule_source


def _classify_unavailable_broker_error(exc: Exception) -> tuple[str, str]:
    return "LIVE_BROKER_NOT_CONFIGURED", str(exc)
...
class OrderRulesSchemaError(RuntimeError):
    """Raised when broker order-rule capability response violates documented schema."""


class OrderRulesMarketMismatchError(OrderRulesSchemaError):
    """Raised when broker order-rule capability response market does not match request market."""
...
def get_effective_order_rules(pair: str, *, notify_fn=None) -> RuleResolution:
    del notify_fn
    cached = get_cached_order_rule_snapshot(pair)
    ...
    resolution = _build_fallback_only_rule_resolution(
        ...
        reason_code="LOCAL_ORDER_RULES_ONLY",
        reason_summary="no live broker adapter is configured",
        reason_detail="order rules are derived only from explicit local settings",
        fallback_risk="live execution unavailable",
    )
    _cached_rules[pair] = (time.time(), resolution, resolution.rules)
    return resolution
