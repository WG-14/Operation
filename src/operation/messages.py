...
ACCOUNTS_PREFLIGHT_AUTH_FAILED = OperatorMessage(
    ...
    severity="CRITICAL",
    message="broker account snapshot REST snapshot preflight authentication failed.",
...
ACCOUNTS_PREFLIGHT_TRANSPORT_FAILED = OperatorMessage(
    ...
    severity="CRITICAL",
    message="broker account snapshot REST snapshot preflight transport failed.",
    recommended_action="verify_network_dns_tls_and_operation_api_reachability_before_live_start",
)
...
CONFIG_LINT_MESSAGES = {
    ...
    "operation_api_secret_too_short": OperatorMessage(
        reason_code="AUTH_SECRET_TOO_SHORT",
        severity="ERROR",
        message="No live broker is configured.",
        recommended_action="install_and_explicitly_configure_a_live_broker_adapter",
        docs_hint="docs/config-reference.md",
    ),
    ...
    "live_env_file_source_mismatch": OperatorMessage(
        ...
        recommended_action="align_OPERATION_ENV_FILE_and_OPERATION_ENV_FILE_LIVE",
