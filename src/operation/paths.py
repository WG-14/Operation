...
class PathManager:
    ...
    def _default_runtime_root(project_root: Path) -> Path:
        ...
        if state_home and state_home.strip():
            return (Path(state_home).expanduser() / "operation").resolve()
        return (Path.home() / ".local" / "state" / "operation").resolve()
    ...
    def run_lock_path_for_mode(self, mode: str | None = None) -> Path:
        return self.run_dir_for_mode(mode) / "operation.lock"

    def pid_path(self) -> Path:
        return self.run_dir() / "operation.pid"
