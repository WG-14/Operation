...
def classify_order_semantics(
    *,
    ...
    if order_type == "price" and normalized_side == "BUY":
        ...
        if exchange_text == "operation" and contract_text == "market_buy_notional":
            evidence_quality = SEMANTIC_EVIDENCE_CURRENT_VERIFIED
        elif contract_text is not None and (exchange_text != "operation" or contract_text != "market_buy_notional"):
            return OrderSemantics(
    ...
    if order_type == "market" and normalized_side == "BUY" and exchange_text == "operation":
        return OrderSemantics(
            raw_order_type=raw_text,
            side=normalized_side,
            exchange=exchange_text,
