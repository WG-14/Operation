...
def _submit_evidence_contract_fields(event: sqlite3.Row | None) -> dict[str, object]:
    ...
    evidence = _decode_submit_evidence(event["submit_evidence"])
    return {
        "exchange": str(evidence.get("exchange") or evidence.get("broker") or "operation").strip().lower() or None,
...
def build_execution_quality_record(
    ...
    *,
    ...
    exchange = str(contract_fields.get("exchange") or "operation").strip().lower() or None
