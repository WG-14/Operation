...
FLAT_STALE_LOT_PROJECTION_REPAIR_COMMAND = (
    "uv run operation rebuild-position-authority --flat-stale-projection-repair --apply --yes"
)
HISTORICAL_FRAGMENTATION_PROJECTION_DRIFT_REPAIR_COMMAND = (
    "uv run operation rebuild-position-authority "
    "--historical-fragmentation-projection-repair --apply --yes"
)
LEGACY_OPERATOR_CLOSEOUT_EVIDENCE_ENRICHMENT_COMMAND = (
    "uv run operation rebuild-position-authority --flat-stale-projection-repair "
...
def build_flat_stale_lot_projection_repair_preview(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        **gate_counts,
        ...
        "preview_command": "uv run operation rebuild-position-authority --flat-stale-projection-repair",
        "touched_tables": [
            "open_position_lots",
