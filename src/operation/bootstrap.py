...
def describe_explicit_env_file(mode: str | None) -> ExplicitEnvLoadSummary:
    ...
    if normalized_mode == "live":
        env_file = os.getenv("OPERATION_ENV_FILE_LIVE")
        return ExplicitEnvLoadSummary(
            mode=normalized_mode,
            env_file=env_file,
            source_key="OPERATION_ENV_FILE_LIVE" if env_file else None,
            loaded=False,
            exists=bool(env_file and os.path.isfile(env_file)),
            override=False,
        )
    if normalized_mode in {"paper", "test"}:
        env_file = os.getenv("OPERATION_ENV_FILE_PAPER")
        return ExplicitEnvLoadSummary(
            mode=normalized_mode,
            env_file=env_file,
            source_key="OPERATION_ENV_FILE_PAPER" if env_file else None,
            loaded=False,
            exists=bool(env_file and os.path.isfile(env_file)),
            override=False,
        )
