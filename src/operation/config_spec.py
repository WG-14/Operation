...
DECLARED_ENV_NAMES: tuple[str, ...] = (
    ...
    "OPERATION_AUTH_DIAGNOSTICS",
    "OPERATION_DEPLOY_COMMIT_SHA",
    "OPERATION_DEPLOY_DIRTY",
    "OPERATION_ENV_FILE",
    "OPERATION_ENV_FILE_LIVE",
    "OPERATION_ENV_FILE_PAPER",
    "OPERATION_OPERATOR_DIRTY_RUNTIME_ACK",
    "OPERATION_PYTEST_RUN_ID",
    "OPERATION_PYTEST_STARTED",
    "OPERATION_PYTEST_SUITE",
    "OPERATION_PYTEST_SUMMARY_ON_SUCCESS",
    "OPERATION_BATCH_CHILD_WORKER_BUDGET",
    "OPERATION_RUNTIME_GIT_DIFF_ARTIFACT_PATH",
    "OPERATION_RUNTIME_GIT_DIFF_HASH",
    "OPERATION_SOURCE_ARCHIVE_HASH",
    "OPERATION_TOTAL_PROCESS_BUDGET",
    "BLOCK_ON_OPEN_ORDER",
...
def _safety_tier_for(name: str) -> SafetyTier:
    if name in LIVE_REQUIRED_KEYS or name.startswith(("LIVE_", "MAX_", "KILL_", "DB_", "RUN_")):
        return "P0"
...
def _category_for(name: str) -> str:
    if name in {"OPERATION_BATCH_CHILD_WORKER_BUDGET", "OPERATION_TOTAL_PROCESS_BUDGET"}:
        return "runtime"
    if name.startswith("OPERATION_PYTEST_") or name.startswith("PYTEST_XDIST_"):
        return "test_runtime"
    if name.endswith("_ROOT") or name in {"DB_PATH", "RUN_LOCK_PATH", "SNAPSHOT_ROOT", "BACKUP_DIR"}:
        return "storage"
    if name.startswith(("OPERATION_", "LIVE_", "MAX_", "KILL_")):
        return "live_safety"
    if name.startswith(("NOTIFIER_", "NTFY_", "SLACK_", "TELEGRAM_", "HEALTH_")):
