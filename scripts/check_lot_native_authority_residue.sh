...
targets=(
  ...
  "src/operation/flatten.py"
  "src/operation/decision_context.py"
  "src/operation/reporting.py"
  "src/operation/recovery.py"
)
...
if rg -n --pcre2 "$forbidden_live_shadow_submit_source" "src/operation/broker/live.py"; then
...
if rg -n --pcre2 "$forbidden_flatten_qty_authority" "src/operation/flatten.py"; then
...
if rg -nUP "$forbidden_live_boundary_authority_override" "src/operation/broker/live.py"; then
...
if rg -nUP "$forbidden_flatten_reserved_exit_rebuild" "src/operation/flatten.py"; then
...
if rg -nUP "$forbidden_mixed_sell_dust_boundary_definition" "src/operation/broker/live.py"; then
...
if rg -nUP "$forbidden_mixed_sell_dust_boundary_call" "src/operation/broker/live.py"; then
