...
- Entry Point Group: `operation.strategy_plugins` or `not_applicable`.
...
- [ ] In-repo built-in strategy plugins updated `src/operation/strategy_plugins/builtin_manifest.py` with a valid Built-in Reason, or external strategy packages declared a `operation.strategy_plugins` entry point.
- [ ] Strategy appears in `list_research_strategy_plugins()` and resolves through `resolve_research_strategy_plugin()`.
- [ ] Strategy plugin inventory verification: `uv run operation-bot strategy-plugin-inventory --json` shows the expected strategy source, contract hash, live eligibility, and fail-closed reason.
- [ ] Strategy plugin validate verification: `uv run operation-bot strategy-plugin-validate --strategy <name> --target <target> --json` checked target verdicts where applicable.
